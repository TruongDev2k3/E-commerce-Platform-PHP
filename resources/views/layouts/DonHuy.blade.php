...
<body>
    <section class="main">
        <div class="list-menu" style="width: 1200px; height:45px; border:solid 1px;">
            ...
            <a href="http://127.0.0.1:8000/OrderCancel" class="title" style="padding-top:5px;text-decoration: none;width: 239px; height:45px;font-size: 20px; color:red; float:left;text-align:center;border-right: 1px solid;">Đơn đã hủy   &nbsp;<i class="fa-solid fa-xmark"></i></a>
        ...
        <div class="row">
            <div class="colf-9 colf-s-12 padding-table" style="padding-left: 0px; width:1300px;">
                ...
                <h2 style="margin-left: 10px; margin-bottom:10px;margin-top:10px;display:block;width:1200px;text-align:left;">Bill ID: {{ $invoice->MaHoaDon }}</h2>
                ...
                @php
                $buttonMarginLeft = $invoice->TrangThai == 'Yêu cầu hủy' ? '1080px' : '970px';
                @endphp
                <button style="cursor: pointer;float:left; @if($invoice->TrangThai == 'Yêu cầu hủy') margin-left: 1080px;@else margin-left: 980px;@endif color:#fff; background-color:orange; border: none; line-height: 35px; font-weight: bold; padding: 3px 10px; font-size: 14px; margin-top: 10px; border-radius:3px;">
                    <span style="color: #fff; font-size:15px;">
                        {{ $invoice->TrangThai }} <i class="fa fa-check"></i>
                    </span>
                </button>
                @if ($invoice->TrangThai != 'Yêu cầu hủy')
                <form action="{{ route('OrderBillCancel', ['id' => $invoice->MaHoaDon]) }}" method="POST" style="display:inline;" onsubmit="return confirmBuy('{{ $invoice->MaHoaDon }}')">
                    @csrf
                    <button type="submit" style="cursor: pointer;margin-left:5px;color:#fff;background-color:green;border: none;line-height: 35px;font-weight: bold;padding: 3px 10px;font-size: 14px;margin-top: 10px;border-radius:3px;">
                        Mua Lại Đơn
                    </button>
                </form>
                @endif
    ...
    <script>
        function confirmBuy(orderId) {
            return confirm(`Bạn có chắc chắn muốn mua lại đơn hàng có ID là ${orderId} không?`);
        }
    </script>
</body>
