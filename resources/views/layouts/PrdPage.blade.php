...
<body>
        ...
        <div class="header grid wide">
            <div class="row">
                ...
                <div class="header__logo__img" style="margin-left: -15px;">
                    <a href="http://127.0.0.1:8000/homepage"><img src="/Images/dev4.png" alt="" style="height: 95%; width: 150px; margin-top: 3px;"></a>
                </div>
                        ...
                        <li class="header__navbar__item">
                            <div class="header__navbar__item__wrapper__last">
                                <a href="" class="header__navbar__item__link">
                                    ...
                                    <div class="header__navbar__item__link__desc__wrapper">
                                        <p id="username-display" style="font-size: 20px;">Xin chào : {{ session('usernames') }}</p>
                        ...
                        <div class="flash__sale__product__desc">
                            ...
                            <div>
                                <form method="POST" action="{{ route('add.to.cart') }}">
                                    @csrf
                                    <input type="hidden" name="PrdID" value="{{ $product->MaSanPham }}">
                                   
                                    <input type="hidden" name="TenSanPham" value="{{ $product->TenSanPham }}">
                                    <input type="hidden" name="AnhSanPham" value="{{ $product->AnhSanPham }}">
                                    <input type="hidden" name="GiaGiam" value="{{ $product->GiaGiam }}">
                                  
                                    
                                    <input type="hidden" name="SoLuong" value="1"> <!-- Đặt giá trị mặc định là 1 -->
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.btn-add-carts').forEach(function (button) {
            button.addEventListener('click', function (event) {
                event.preventDefault(); // Ngăn chặn hành động mặc định của nút submit

                // Lấy thông tin sản phẩm từ các trường ẩn
                var productId = this.closest('form').querySelector('input[name="productId"]').value;

                // Kiểm tra xem sản phẩm đã tồn tại trong giỏ hàng chưa
                var cartItems = JSON.parse(localStorage.getItem('cartItems')) || [];
                var isProductExist = cartItems.some(function (item) {
                    return item.productId === productId;
                });

                // Nếu sản phẩm đã tồn tại trong giỏ hàng, hiển thị thông báo và ngăn chặn thêm sản phẩm mới
                if (isProductExist) {
                ...
                var productName = this.closest('form').querySelector('input[name="productName"]').value;
                var productImage = this.closest('form').querySelector('input[name="productImage"]').value;
                var productPrice = this.closest('form').querySelector('input[name="productPrice"]').value;
                var productOriginalPrice = this.closest('form').querySelector('input[name="productOriginalPrice"]').value;
                var productSoldQuantity = this.closest('form').querySelector('input[name="productSoldQuantity"]').value;
                var productQuantity = this.closest('form').querySelector('input[name="productQuantity"]').value;
                ...
                // Lưu mảng giỏ hàng mới vào local storage
                localStorage.setItem('cartItems', JSON.stringify(cartItems));

                // Hiển thị thông báo thành công hoặc chuyển hướng người dùng đến trang giỏ hàng
